        ...
        for (i=0;i<m_testOrders.getOrdersCount();i++) {
            switch (m_testOrders.getTestOrder(i)->m_type) {
                ...
                case IsButtonDownCount:
                    if (m_mainWindow->getButton(m_testOrders.getTestOrder(i)->m_listNumber.at(0))->getMouseDownCount() !=
                ...
                case isHoverCount:
                    ...
                    break;
                case setVisibleItem:
                    if (m_testOrders.getTestOrder(i)->m_listNumber.at(0) >= 6) {
                        m_mainWindow->getItem2d( m_testOrders.getTestOrder(i)->m_listNumber.at(0) )->setVisible( m_testOrders.getTestOrder(i)->m_listNumber.at(1) );
                    }
                ...
                case getMouseCursorOnHover:
                    if (m_mainWindow->getButton( m_testOrders.getTestOrder(i)->m_listNumber.at(0) )->getMouseCursorOnHover() != m_testOrders.getTestOrder(i)->m_listNumber.at(1)) {
                        TG_ERROR_LOG("getButton change is incorrect, index: ", m_testOrders.getTestOrder(i)->m_lineNumber);
                        m_returnIndex = 1;
                        m_mainWindow->exit();
                        return;
                    }
                ...
                case setEnabledItem:
                    if (m_testOrders.getTestOrder(i)->m_listNumber.at(0) >= 6) {
                        m_mainWindow->getItem2d( m_testOrders.getTestOrder(i)->m_listNumber.at(0) )->setEnabled( m_testOrders.getTestOrder(i)->m_listNumber.at(1) );
                    }
                    std::this_thread::sleep_for(std::chrono::milliseconds( 100 ) );
                    break;
                case isEnabled:
                ...
                case TestOrderType::isMouseClicked:
                    ...
                    break;
                case TestOrderType::isMouseMenuClicked:
                    if (!checkIsMouseMenuClicked(m_testOrders.getTestOrder(i)->m_listNumber.at(0),
                        m_testOrders.getTestOrder(i)->m_listNumber.at(1),
                        m_testOrders.getTestOrder(i)->m_listNumber.at(2))) {
                        TG_ERROR_LOG("isMouseMenuClicked change is incorrect, index: ", m_testOrders.getTestOrder(i)->m_lineNumber);
                        m_returnIndex = 1;
                        m_mainWindow->exit();
                        return;
                    }
                    break;
                case TestOrderType::setSelected:
                    m_mainWindow->getButton( m_testOrders.getTestOrder(i)->m_listNumber.at(0) )->setSelected( m_testOrders.getTestOrder(i)->m_listNumber.at(1) );
                    break;
                case TestOrderType::isItemSelected:
                    if (m_mainWindow->getButton( m_testOrders.getTestOrder(i)->m_listNumber.at(0) )->getSelected() != m_testOrders.getTestOrder(i)->m_listNumber.at(1)) {
                        TG_ERROR_LOG("Item is not selected, index: ", m_testOrders.getTestOrder(i)->m_lineNumber);
                        m_returnIndex = 1;
                        m_mainWindow->exit();
                        return;
                    }
...
bool FunctionalTest::checkIsMouseMenuClicked(int index, int x, int y)
{
    if (m_mainWindow->getMouseStateChangeCount() == m_latestHoverIndex) {
        return false;
    }
    HoverVisibleChangeState wantedState = HoverVisibleChangeState::MouseMenuClicked;
    if (m_mainWindow->getMouseStateChange(m_latestHoverIndex)->m_index != static_cast<size_t>(index) || m_mainWindow->getMouseStateChange(m_latestHoverIndex)->m_x != x || m_mainWindow->getMouseStateChange(m_latestHoverIndex)->m_y != y || m_mainWindow->getMouseStateChange(m_latestHoverIndex)->m_state != wantedState)
    {
        TG_ERROR_LOG("MouseClicked change is incorrect ", m_latestHoverIndex, index, x, y,
                     m_mainWindow->getString(wantedState),
