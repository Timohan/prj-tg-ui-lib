    ...
    if (ordersFile.is_open()) {
        ...
        while (std::getline(ordersFile, line)) {
            ...
            if (line.compare(0, 4, "MMC ") == 0) {
                ...
                m_listOrder.push_back(orders);
            } else if (getNextText(line) == "removeMenu") {
                orders.m_type = TestOrderType::RemoveMenu;
                textPos = getNextText(line).size()+1;
                for (i=0;i<3;i++) {
                    std::string text = getNextText(line.c_str()+textPos);
                    if (text.size() == 0) {
                ...
                m_listOrder.push_back(orders);
            } else if (getNextText(line) == "isMouseMenuClicked") {
                orders.m_type = TestOrderType::isMouseMenuClicked;
                textPos = getNextText(line).size()+1;
                for (i=0;i<3;i++) {
                    std::string text = getNextText(line.c_str()+textPos);
                    if (text.size() == 0) {
