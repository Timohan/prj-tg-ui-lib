        ...
        for (i=0;i<m_testOrders.getOrdersCount();i++) {
            switch (m_testOrders.getTestOrder(i)->m_type) {
                ...
                case setSelected:
                    break;
                case isItemSelected:
                    if (m_mainWindow->isSelected(m_testOrders.getTestOrder(i)->m_listNumber.at(0))
                        != m_testOrders.getTestOrder(i)->m_listNumber.at(1)) {
                        TG_ERROR_LOG("isItemSelected is not correct index: ",
                            m_testOrders.getTestOrder(i)->m_listNumber.at(0),
                            " Should be: ",
                            m_testOrders.getTestOrder(i)->m_listNumber.at(1),
                            " Line number: ",
                            m_testOrders.getTestOrder(i)->m_lineNumber
                            );
                        m_returnIndex = 1;
                        m_mainWindow->exit();
                        return;
                ...
                case TestOrderType::MakeStep:
                    ...
                    break;
                case TestOrderType::TypeKeyPress:
                    sendKeyPress(static_cast<uint32_t>(m_testOrders.getTestOrder(i)->m_listNumber.at(0)), 1, true, false);
                    break;
                case TestOrderType::TypeKeyRelease:
                    sendKeyPress(static_cast<uint32_t>(m_testOrders.getTestOrder(i)->m_listNumber.at(0)), 1, false, true);
                    break;
                ...
                case TestOrderType::isImage:
                    ...
                    break;
                case TestOrderType::isComboBoxItemChanged:
                    if (!checkIsComboBoxItemChanged(m_testOrders.getTestOrder(i)->m_listNumber.at(0),
                        m_testOrders.getTestOrder(i)->m_listNumber.at(1))) {
                        TG_ERROR_LOG("isComboBoxItemChanged change is incorrect, index: ", m_testOrders.getTestOrder(i)->m_lineNumber);
                        m_returnIndex = 1;
                        m_mainWindow->exit();
                        return;
                    }
...
bool FunctionalTest::checkIsComboBoxItemChanged(size_t comboBoxIndex, size_t itemIndex)
{
    if (m_mainWindow->getMouseStateChangeCount() == m_latestHoverIndex) {
        return false;
    }
    HoverVisibleChangeState wantedState = HoverVisibleChangeState::ComboBoxItemChanged;
    if (m_mainWindow->getMouseStateChange(m_latestHoverIndex)->m_index != static_cast<size_t>(comboBoxIndex)
        || m_mainWindow->getMouseStateChange(m_latestHoverIndex)->m_itemIndex != itemIndex
        || m_mainWindow->getMouseStateChange(m_latestHoverIndex)->m_state != wantedState) {
        TG_ERROR_LOG("ComboBoxItemChanged change is incorrect ", m_latestHoverIndex, comboBoxIndex, itemIndex,
            m_mainWindow->getString(wantedState),
            m_mainWindow->getMouseStateChange(m_latestHoverIndex)->m_index,
            m_mainWindow->getMouseStateChange(m_latestHoverIndex)->m_itemIndex);
        return false;
    }
    m_latestHoverIndex++;
    return true;
