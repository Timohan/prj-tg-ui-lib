    ...
    if (ordersFile.is_open()) {
        ...
        while (std::getline(ordersFile, line)) {
            ...
            if (line.compare(0, 4, "MMC ") == 0) {
                ...
                orders.m_type = TestOrderType::TypeKeyPress;
                textPos = getNextText(line).size()+1;
                for (i=0;i<1;i++) {
                    std::string text = getNextText(line.c_str()+textPos);
                    if (text.size() == 0) {
                    ...
                    }  else if (text == "XK_KP_Up") {
                        orders.m_listNumber.push_back(XK_Up);
                    } else if (text == "XK_KP_Down") {
                        orders.m_listNumber.push_back(XK_Down);
                    } else {
                        orders.m_listNumber.push_back(std::atoi(text.c_str()));
                    }
                    textPos += text.size() + 1;
                ...
                m_listOrder.push_back(orders);
            } else if (getNextText(line) == "KeyRelease") {
                orders.m_type = TestOrderType::TypeKeyRelease;
                textPos = getNextText(line).size()+1;
                for (i=0;i<1;i++) {
                    std::string text = getNextText(line.c_str()+textPos);
                    if (text.size() == 0) {
                ...
                m_listOrder.push_back(orders);
            }  else if (getNextText(line) == "Sleep") {
                orders.m_type = TestOrderType::SleepWaitTimeMs;
                textPos = getNextText(line).size()+1;
                for (i=0;i<1;i++) {
                    std::string text = getNextText(line.c_str()+textPos);
                    if (text.size() == 0) {
                ...
                m_listOrder.push_back(orders);
            } else if (getNextText(line) == "isComboBoxItemChanged") {
                orders.m_type = TestOrderType::isComboBoxItemChanged;
                textPos = getNextText(line).size()+1;
                for (i=0;i<2;i++) {
                    std::string text = getNextText(line.c_str()+textPos);
                    if (text.size() == 0) {
