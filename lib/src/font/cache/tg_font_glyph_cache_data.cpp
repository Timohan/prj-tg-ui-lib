...
/*!
 * \brief TgFontGlyphCacheData::generateTextVertices
 *
 * some basic info
 *
 ...
 */
bool TgFontGlyphCacheData::generateTextVertices(TgFontInfoData *newInfo, const std::vector<uint32_t> &listCharacters)
{
    TG_FUNCTION_BEGIN();
    ...
    for (i=0;i<c;i++) {
        ...
        y = bottomY - static_cast<float>(glyph->image_pixel_bottom_y-glyph->image_pixel_top_y) - static_cast<float>(glyph->image_pixel_offset_line_y);

        newInfo->m_listTopPositionY.push_back(y);
